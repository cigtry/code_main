...
{
	u8 sta,len;  
	if((UART->STATUS.BITS.RRDY))//串口接收完成？
	{					   
		len=uart.receive_count;	//得到此次接收到的数据长度
		uart.receive_buffer[len]='\0';	//在末尾加入结束符.
		sta=usmart_dev.cmd_rec(uart.receive_buffer);//得到函数各个信息
		if(sta==0)usmart_dev.exe();	//执行函数
		else 
		{  
			len=usmart_sys_cmd_exe(uart.receive_buffer);
		...
		(UART->STATUS.BITS.RRDY)=0;//状态寄存器清空
	}
}
