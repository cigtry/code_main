...
//mt_mov_mode==5'd0->上电复位           mt_mov_mode==5'd1->正常复位
//mt_mov_mode==5'd2->正向运动           mt_mov_mode==5'd3->反向运动 
//mt_mov_mode==5'd4->正向速度模式       mt_mov_mode==5'd5->反向速度模式 
//mt_mov_mode==5'd6->正向限制速度模式   mt_mov_mode==5'd7->反向限制速度模式 
  always @(*)begin
    case(state_c)
      IDLE       : begin
        if(pos_start)begin
      ...
      ACC_STATE  : begin
        ...
        else if (acc2dec) begin
          state_n = DEC_STATE;
        end
        else if(acc2keep)begin
          state_n = KEEP_STATE;
        end
        else begin
          state_n = state_c;
...
//即t=100_000_000*T=100M*10ns=1s,以1s为基准，参考速度变快或者变慢
  ...
  localparam                                         CLK_FRQE       = 28'd100_000;
  reg            [  27:00]                       v_unit          ;
  reg            [  27:00]                       s_unit          ;
  ...
  always @(posedge clk or negedge rst_n) begin
    ...
    else if((move_mode == 5'd1) && (position_set < (zero_position + QUIT_LONG)) && start)begin
  ...
  always @(posedge clk ) begin
    if ((!short_normal_rst_flag) && (move_mode_r == 5'd1)) begin
...
//========================================================//
  ...
  always @(posedge clk or negedge rst_n) begin
    ...
    else if((move_mode_r == 5'd1)&&(mt_current_s ==position + OPT_LONG + QUIT_LONG )&& (!opt_out2in_ok)  )begin
  ...
  always @(posedge clk or negedge rst_n) begin
    ...
    else if((move_mode_r == 5'd0)&&(mt_current_s ==position )&& (!opt_out2in_ok) && (state_c == IN_OPT) )begin
      in_opt_error <= 1'b1;
    end
    else begin
      in_opt_error <= 1'b0;
  ...
  always @(posedge clk or negedge rst_n) begin
    ...
    else if((move_mode_r == 5'd1) && (state_c == ACC_STATE) && (opt_in2out|| opt_out2in))begin
      acc2error <= 1'b1;
    end
    else begin
      acc2error <= 1'b0;
  ...
  always @(posedge clk or negedge rst_n) begin
    ...
    else if((move_mode_r == 5'd1) && (state_c == KEEP_STATE) && (opt_in2out|| opt_out2in))begin
      keep2error <= 1'b1;
    end
    else begin
      keep2error <= 1'b0;
  ...
  always @(posedge clk or negedge rst_n) begin
    ...
    else if((move_mode_r == 5'd1) && (state_c == DEC_STATE) && opt_out2in && (abs_position >= 400))begin
      dec2err <= 1'b1;
    end
    else begin
      dec2err <= 1'b0;
  ...
  always @(posedge clk or negedge rst_n) begin
    ...
    else if((move_mode_r == 5'd1) && (dec_current_s == {(BIT_S - 1){1'b0}}) && start_gen_step)begin
      dec2in_opt <= 1'b1;
    end
    else begin
      dec2in_opt <= 1'b0;
  ...
  always @(posedge clk or negedge rst_n) begin
    ...
    else if(dec_flag && (mt_current_s <= start_speed))begin
      dec2end <= 1'b1;
    end
    else if(((move_mode_r == 5'd2)||(move_mode_r == 5'd3)) && (dec_current_s == {(BIT_S - 1){1'b0}}) && start_gen_step)begin
      dec2end <= 1'b1;
    end
    else begin
      dec2end <= 1'b0;
...
/*reg                                            speed_mode_acc2keep  ;//速度模式 加速到匀速
  ...
  always @ (posedge clk or negedge rst_n)begin
    ...
    else if((move_mode_r > 5'd3) && (state_c == ACC_STATE) && (mt_current_v == target_speed))begin
  ...
  always @ (posedge clk or negedge rst_n)begin
    ...
    else if((move_mode_r > 5'd3) && (state_c == KEEP_STATE) && (mt_current_v > target_speed))begin
  ...
  always @ (posedge clk or negedge rst_n)begin
    ...
    else if((move_mode_r > 5'd3) && (state_c == DEC_STATE) && (mt_current_v == target_speed))begin
  ...
  always @ (posedge clk or negedge rst_n)begin
    ...
    else if((move_mode_r > 5'd5) && (mt_current_s == position) && start_gen_step)begin
...
always @(posedge clk or negedge rst_n)
    begin
        if(!rst_n)
            abs_position                 <= 32'd0;
        else if(((move_mode_r == 5'd1) ||(move_mode_r == 5'd0))&& mt_success_int)begin
            abs_position                 <= 32'd0;
        end
    ...
    always @(posedge clk ) begin
      case(state_c)
          ...
          ACC_STATE ,KEEP_STATE, DEC_STATE :  begin
            if ((move_mode_r == 5'd1) ||(move_mode_r == 5'd3) || (move_mode_r == 5'd5) ||(move_mode_r == 5'd7)) begin
              dirction <= ~set_dir;
            end
            else begin
              dirction <= set_dir;
